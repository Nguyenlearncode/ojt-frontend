import React, { useState, useMemo, useEffect } from "react";
import {
  Box,
  Container,
  Heading,
  Text,
  Button,
  Flex,
  VStack,
  Spinner,
  Alert,
  AlertIcon,
  HStack,
} from "@chakra-ui/react";
import { motion } from "framer-motion";
import { useNavigate } from "react-router-dom";
import { FiPlus, FiShield } from "react-icons/fi";
import SearchFilterRoleChakra from "../components/SearchFilterRoleChakra";
import { RoleTableChakra } from "../components/RoleTableChakra";
import { useRoles } from "../hooks/useRoles";
import { useDeleteRole } from "../hooks/useDeleteRole";
import type { Role } from "../api/roleApi";

const MotionBox = motion(Box);

const RoleManagementPageChakra: React.FC = () => {
  const { roles, loading, error, refetch } = useRoles();
  const { deleteRole } = useDeleteRole();
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState("");

  // Auto refetch when window gets focus
  useEffect(() => {
    const handleFocus = () => refetch && refetch();
    window.addEventListener("focus", handleFocus);
    return () => window.removeEventListener("focus", handleFocus);
  }, [refetch]);

  // Filter roles
  const filteredRoles = useMemo(() => {
    if (!roles || roles.length === 0) return [];
    return roles.filter((role) => {
      const matchesSearch =
        role.roleName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        role.roleCode?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (role.roleDescription && role.roleDescription.toLowerCase().includes(searchTerm.toLowerCase()));
      return matchesSearch;
    });
  }, [roles, searchTerm]);

  // Edit handler
  const handleEdit = (role: Role) => {
    navigate(`/UpdateRole/${role.roleCode}`);
  };

  // Create handler
  const handleCreate = () => {
    navigate("/CreateRole");
  };

  // Delete handler
  const handleDelete = async (roleCode: string) => {
    await deleteRole(roleCode, () => {
      refetch();
    });
  };

  // Loading state
  if (loading) {
    return (
      <Flex minH="100vh" align="center" justify="center" direction="column" gap={4}>
        <Spinner
          thickness="4px"
          speed="0.65s"
          emptyColor="gray.200"
          color="brand.500"
          size="xl"
        />
        <Text color="gray.600">Đang tải danh sách roles...</Text>
      </Flex>
    );
  }

  // Error state
  if (error) {
    return (
      <Container maxW="container.md" py={10}>
        <Alert
          status="error"
          variant="subtle"
          flexDirection="column"
          alignItems="center"
          justifyContent="center"
          textAlign="center"
          minH="200px"
          borderRadius="xl"
        >
          <AlertIcon boxSize="40px" mr={0} />
          <Text mt={4} mb={1} fontSize="lg" fontWeight="bold">
            {error}
          </Text>
          <Button colorScheme="blue" mt={4} onClick={() => refetch()}>
            Thử lại
          </Button>
        </Alert>
      </Container>
    );
  }

  return (
    <Box minH="100vh" py={8}>
      <Container maxW="container.xl">
        {/* Header */}
        <MotionBox
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          mb={8}
        >
          <Flex justify="space-between" align="center" flexWrap="wrap" gap={4}>
            <VStack align="flex-start" spacing={1}>
              <HStack spacing={3}>
                <Box
                  bg="whiteAlpha.300"
                  p={3}
                  borderRadius="lg"
                  color="white"
                  backdropFilter="blur(10px)"
                >
                  <FiShield size={28} />
                </Box>
                <Box>
                  <Heading
                    size="xl"
                    bgGradient="linear(to-r, pink.300, white, blue.300)"
                    bgClip="text"
                    fontWeight="extrabold"
                  >
                    Quản lý Roles
                  </Heading>
                  <Text
                    bgGradient="linear(to-r, whiteAlpha.900, pink.200)"
                    bgClip="text"
                    fontSize="md"
                    textShadow="0px 1px 2px rgba(0,0,0,0.2)"
                  >
                    Quản lý vai trò và quyền hạn trong hệ thống
                  </Text>
                </Box>
              </HStack>
            </VStack>
            <MotionBox whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                leftIcon={<FiPlus />}
                variant="gradient"
                size="lg"
                onClick={handleCreate}
              >
                Tạo Role mới
              </Button>
            </MotionBox>
          </Flex>
        </MotionBox>

        {/* Search & Filter */}
        <SearchFilterRoleChakra
          searchTerm={searchTerm}
          onSearchChange={setSearchTerm}
        />

        {/* Table */}
        <RoleTableChakra
          roles={filteredRoles}
          onEdit={handleEdit}
          onDelete={handleDelete}
        />
      </Container>
    </Box>
  );
};

export default RoleManagementPageChakra;

