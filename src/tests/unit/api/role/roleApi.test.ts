...
describe("🧩 roleApi", () => {
  ...
  // getAllRoles
  it("trả về danh sách roles (response.data là mảng)", async () => {
    (axiosClient.get as any).mockResolvedValue({
      data: [{ roleCode: "ADMIN", roleName: "Admin", privileges: [] }],
    });

    const result = await roleApi.getAllRoles();
    expect(result).toHaveLength(1);
    expect(result[0].roleCode).toBe("ADMIN");
    expect(axiosClient.get).toHaveBeenCalledWith("/iam/role/all");
  });

  it("trả về danh sách roles khi response là mảng", async () => {
    (axiosClient.get as any).mockResolvedValue([
  ...
  it("gọi đúng endpoint khi tạo role", async () => {
    ...
    expect(axiosClient.post).toHaveBeenCalledWith("/iam/role/create", payload);
  });
  ...
  it("gọi đúng endpoint khi cập nhật role", async () => {
    ...
    expect(result).toEqual(payload);
    expect(axiosClient.post).toHaveBeenCalledWith("/iam/role/update", payload);
  });
  ...
  it("gọi đúng endpoint khi xóa role", async () => {
    ...
    expect(axiosClient.delete).toHaveBeenCalledWith("/iam/role/delete/ADMIN");
  });
});
